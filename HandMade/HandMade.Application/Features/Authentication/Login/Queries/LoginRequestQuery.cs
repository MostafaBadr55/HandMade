...
namespace HandMade.Application.CQRS.Authentication.Login.Queries
{
    ...
    public class LoginRequestQueryHandler(IAccountServices authService, IAuthTokenService tokenService) : IRequestHandler<LoginRequestQuery, RequestResult<LoginResponseDTO>>
    {
        public async Task<RequestResult<LoginResponseDTO>> Handle(LoginRequestQuery request, CancellationToken cancellationToken)
        {
            ...
            if (user is null)
                return RequestResult<LoginResponseDTO>.Failed(ErrorCode.InvalidUsernameOrPassword);

            if (!user.IsActive)
                return RequestResult<LoginResponseDTO>.Failed(ErrorCode.InActiveAccount);
